...
-- BETWEEN (2)
	...
	#Finally, modify to your previous query to include all Spanish language or French language films with the same criteria as before. Dont forget your parentheses!
	SELECT title, release_year
	FROM films
	WHERE release_year BETWEEN 1990 AND 2000
	AND budget > 100000000
...
-- NULL and IS NULL
	...
	#Get the title of every film which doesnt have a budget associated with it.
	SELECT title
	FROM films
	WHERE budget IS null;

	#Get the number of films which dont have a language associated with them.
	SELECT COUNT(*)
	FROM films
...
-- LIKE and NOT LIKE
	...
	#Get the names of people whose names dont start with A. The pattern you need is 'A%'.
	SELECT name
	FROM people
...
--It's AS simple AS aliasing
	...
	#Get the title and duration in hours for all films. The duration is in minutes, so youll need to divide by 60.0 to get the duration in hours. Alias the duration in hours as duration_hours
